...
class CartesianFnTest : public ::testing::Test {
protected:
    CartesianFnTest() : _fn(&_real_fn){
    }
    virtual void SetUp() {
    ...
    CartesianFn _real_fn;
    PyFunctorCaller _fn;
    flume::MockEmitter<boost::python::object> _emitter;
};

TEST_F(CartesianFnTest, TestNormal) {
    try {
...
TEST_F(CartesianFnTest, TestEmpty) {
    try {
        std::vector<boost::python::object> side_input_vec;
        std::auto_ptr<flume::core::Iterator> it =
                baidu::bigflow::python::iterator(side_input_vec.begin(), side_input_vec.end());
