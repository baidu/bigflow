...
#ifndef BIGFLOW_PYTHON_TUPLE_SERDE_H_
#define BIGFLOW_PYTHON_TUPLE_SERDE_H_
...
class TupleSerde : public PythonObjector {
public:
    TupleSerde(){}
    virtual ~TupleSerde(){
        for (size_t i = 0; i < _tuple_element_serde.size(); ++i) {
            _tuple_element_serde[i].release_all();
        }
        _tuple_element_serde.clear();
    };
...
private:
    std::vector<SerdeImplWrapper> _tuple_element_serde;
};
...
#endif  // BIGFLOW_PYTHON_TUPLE_SERDE_H_
